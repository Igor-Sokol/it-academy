...
public class PlayerController : MonoBehaviour
{
    ...
    [SerializeField] private SoundController stepSound;
    [SerializeField] private float secondsToStep;
    ...
    private Vector3 Movement()
    {
        ...
        if (_characterController.isGrounded && movement.sqrMagnitude > 0)
        {
            _stepTimer += Time.deltaTime;
            if (_stepTimer >= secondsToStep)
            {
                stepSound.Play();
                
                _stepTimer -= secondsToStep;
            }
        }
